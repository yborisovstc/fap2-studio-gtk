...
class SDesLog: public MDesLog
{
	...
	virtual bool IsNodeLogged(MElem* aNode, TLogRecCtg aCtg) const;
    protected:
	TLog mLog;
};
...
class LogViewL: public TreeView
{
	...
	void Select(MElem* aNode, TLogRecCtg aCtg);
	tSigLogRecActivated SignalLogRecActivated() {return mSigLogRecActivated;};
    protected:
	void on_log_added(long aTimeStamp, TLogRecCtg aCtg, const MElem* aNode, int aMutId, const std::string& aContent);
	void on_tlog_added(const TLog& aLog);
	void on_des_env_changed();
	static const string& CtgName(TLogRecCtg aCtg); 
